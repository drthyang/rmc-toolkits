...
// Marching cubes over a scalar field on a regular grid. Extracts the isosurface
// at a given level as an indexed-free triangle soup with per-vertex normals from
// the field gradient, so a KDE density volume can be shown as a 3D surface.
//
// Three.js ships a MarchingCubes helper, but it is a metaball/blob generator
// that rebuilds a field from point sources -- it cannot contour an existing
// volume. This is the field-contouring version the PCA-KDE view needs. Vertices
// come out in grid-index space [0..nx-1] x [0..ny-1] x [0..nz-1]; the caller
// maps them to Cartesian through the KDE's axis coordinates and principal axes.
//
// The edge and triangle tables are the classic Paul Bourke / Lorensen-Cline
// lookup tables (public-domain algorithm data).
...
// Each row lists triangles (triplets of edge indices), terminated by -1.
// prettier-ignore
const TRI_TABLE = [
...
// Edge endpoints as indices into CORNER_OFFSETS.
const EDGE_CORNERS = [
    [0, 1], [1, 2], [2, 3], [3, 0],
    [4, 5], [5, 6], [6, 7], [7, 4],
    [0, 4], [1, 5], [2, 6], [3, 7]
...
/**
 * Extract the `isoLevel` isosurface of a scalar field.
 *
 * @param {Float64Array|Float32Array|number[]} field  scalar values, C order over
 *   (x, y, z): index = (i * ny + j) * nz + k.
 * @param {number} nx @param {number} ny @param {number} nz  grid dimensions.
 * @param {number} isoLevel
 * @returns {{ positions: Float32Array, normals: Float32Array, count: number }}
 *   vertices in grid-index space; every three vertices form a triangle.
 */
export const marchingCubes = (field, nx, ny, nz, isoLevel) => {
    const at = (i, j, k) => field[(i * ny + j) * nz + k];
    // Central-difference gradient (for normals), clamped at the borders.
    const grad = (i, j, k) => {
        const gx = at(Math.min(i + 1, nx - 1), j, k) - at(Math.max(i - 1, 0), j, k);
        const gy = at(i, Math.min(j + 1, ny - 1), k) - at(i, Math.max(j - 1, 0), k);
        const gz = at(i, j, Math.min(k + 1, nz - 1)) - at(i, j, Math.max(k - 1, 0));
        return [gx, gy, gz];
    };

    const positions = [];
    const normals = [];
    const cornerValue = new Array(8);
    const cornerPos = new Array(8);
    const cornerGrad = new Array(8);
    const edgeVertex = new Array(12);
    const edgeNormal = new Array(12);
        ...
        for (let j = 0; j < ny - 1; j += 1) {
            for (let k = 0; k < nz - 1; k += 1) {
                ...
                for (let c = 0; c < 8; c += 1) {
                    ...
                    const cj = j + oj;
                    const ck = k + ok;
                    const value = at(ci, cj, ck);
                    cornerValue[c] = value;
                    cornerPos[c] = [ci, cj, ck];
                    cornerGrad[c] = null;
                    if (value < isoLevel) cubeIndex |= 1 << c;
                }
                ...
                for (let e = 0; e < 12; e += 1) {
                    ...
                    const denom = vb - va;
                    const t = Math.abs(denom) < 1e-12 ? 0.5 : (isoLevel - va) / denom;
                    ...
                    if (!cornerGrad[a]) cornerGrad[a] = grad(pa[0], pa[1], pa[2]);
                    if (!cornerGrad[b]) cornerGrad[b] = grad(pb[0], pb[1], pb[2]);
                    const ga = cornerGrad[a];
                    const gb = cornerGrad[b];
                    edgeNormal[e] = [
                        ga[0] + t * (gb[0] - ga[0]),
                        ga[1] + t * (gb[1] - ga[1]),
                        ga[2] + t * (gb[2] - ga[2])
                ...
                for (let t = 0; t < tri.length; t += 3) {
                    for (let corner = 0; corner < 3; corner += 1) {
                        const vertex = edgeVertex[tri[t + corner]];
                        const normal = edgeNormal[tri[t + corner]];
                        positions.push(vertex[0], vertex[1], vertex[2]);
                        // Field increases inward for a density blob, so the outward
